          ...
          <div className="flex flex-row flex-wrap items-stretch justify-center w-full gap-8">
            {section.data.service.map((s, index) => {
              ...
              if (s.LinkPage?.at(0) === "/") {
                ...
                const sec = allSections.find((s) => s.SectionId === sectionId);
                l = "#" + sec?.name;
              }
              if (dev) l = "";
              ...
              return (
                <motion.div
                  ...
                  whileInView={anim && mounted ? "show" : {}}
                  key={index}
                  className="group w-[340px] max-w-full flex flex-col rounded-2xl overflow-hidden bg-white border border-black/[0.06] shadow-[0_10px_30px_-18px_rgba(0,0,0,0.35)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_28px_50px_-24px_rgba(0,0,0,0.4)]"
                >
                  ...
                  <div className="flex flex-col flex-1 p-6 gap-3">
                    <h3 className="text-[22px] font-display font-bold tracking-[-0.01em] text-primaryDesign">
                      {s.name}
                    </h3>
                    <div
                      className="text-[15px] leading-relaxed text-textDesign/75 line-clamp-3"
                      dangerouslySetInnerHTML={{ __html: s.description || "" }}
                    />
                    <a
                      href={dev ? undefined : l ? l : undefined}
                      className="group/link mt-auto inline-flex items-center gap-1.5 font-medium w-fit pt-1"
                    >
                      <span className="bg-gradient-to-r from-primaryDesign to-accentDesign bg-clip-text text-transparent">
                        Scopri di più
                      </span>
                      <ChevronRight className="w-4 h-4 text-accentDesign transition-transform duration-300 group-hover/link:translate-x-1" />
                    </a>
                  </div>
                </motion.div>
              );
