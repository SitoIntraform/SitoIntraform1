...
import SelectOneImageForConfigurator from "./SelectOneImageForConfigurator";
...
function ServiceConfigurator({
  ...
  const onChaneName = (index: number, value: string) => {
    const s = service;
    s[index].name = value;
    setService([...s]);
  };
  ...
  const onChangeImage = (index: number, value: string) => {
    const s = service;
    s[index].image = value;
    setService([...s]);
  };
  ...
  return (
    <div className="my-[16px]">
      ...
      <HeaderPage
        title="Serivizi"
        description="Gestisci l'elenco dei servizi della pagina"
      >
        <Button onClick={onAddService} width={170} height={55} animation>
            ...
            >
              ...
              <div className="w-full space-y-[16px]">
                <Input
                  label="Nome servizio"
                  value={s.name}
                  onValueChange={(e) => onChaneName(index, e.target.value)}
                />
                <Input
                  label="Descrizione servizio"
                  value={s.description}
                  onValueChange={(e) => onChangeDescription(index, e.target.value)}
                />
                <SelectOneImageForConfigurator 
                    title={s.name}
                    totalImage={totalImage}
                    images={[s.image]}
                    setImages={(images) => {
                        onChangeImage(index, images[0])
                    }}
                />
              </div>
              <Trash
